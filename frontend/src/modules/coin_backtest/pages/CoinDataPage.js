...
export default function CoinDataPage() {
  ...
  const [lastRefresh, setLastRefresh] = useState(null); // 마지막 새로고침 시각
  ...
  async function refreshTickerList() {
    const data = await fetchBithumbKrwTickers();
    setTickers(data.tickers || []);
    setLastRefresh(new Date()); // 새로고침 시각 기록!
  }
  ...
  // === "현재 데이터가 있는 종목" 계산 ===
  // dataList 내 각 market이 중복 없이 몇 개인지 계산
  const dataTickers = Array.from(new Set(dataList.map(d => d.market)));
  const totalTickers = tickers.length;            // 전체 원화종목 개수
  const totalDataTickers = dataTickers.length;    // 현재 데이터가 있는 종목 개수
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      onClick={() => navigate("/coin_backtest")}
    >
      메인화면으로 이동
    </button>
      <div style={{ fontSize: 14, marginBottom: 8, color: "#666" }}>
        <b>
          다운로드 기준: 일봉 730개(2년), 4시간봉 360개(2달), 1시간봉 720개(1달), 10분봉 4320개(1달), 3분봉 7200개(15일), 1분봉 7200개(5일)
        </b>
        <br />
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          {/* === 개수 정보 === */}
          <span style={{marginLeft:24, fontSize:15, color:"#333"}}>
            <b>전체 원화종목:</b> {totalTickers}개 &nbsp;|&nbsp; 
            <b>데이터 보유 종목:</b> {totalDataTickers}개
          </span>
          <span style={{marginLeft:18, color:"#555", fontSize:13}}>
            {lastRefresh && (
              <>
                마지막 전체종목 리스트 재업데이트: 
                <b> {lastRefresh.toLocaleString("ko-KR", { hour12: false })}</b>
              </>
            )}
          </span>
          {progress.total > 0 && (
                    ...
                    <>
                      <span style={{ color: "green", fontSize: 13 }}>
                        {files.map((data, idx) =>
                          data ? (
                            <span key={idx}>
                              {INTERVALS[idx].label}: {data.row_count}개, {data.size_mb}MB, {toKoreanTime(data.last_modified)}
                              <br />
                            </span>
                          ) : null
                        )}
